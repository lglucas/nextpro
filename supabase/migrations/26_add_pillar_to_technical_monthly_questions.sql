alter table public.technical_monthly_questions
add column if not exists pillar text;

update public.technical_monthly_questions
...
alter table public.technical_monthly_questions
drop constraint if exists technical_monthly_questions_pillar_check;

alter table public.technical_monthly_questions
add constraint technical_monthly_questions_pillar_check
...
set pillar = case
  ...
  when prompt ilike '%mental%' or prompt ilike '%emocional%' or prompt ilike '%coragem%' or prompt ilike '%confiança%' or prompt ilike '%confianca%' or prompt ilike '%maturidade%' or prompt ilike '%competitiv%' or prompt ilike '%liderança%' or prompt ilike '%lideranca%' or prompt ilike '%comunica%' or prompt ilike '%profissional%' or prompt ilike '%disciplina%' or prompt ilike '%assiduidade%' or prompt ilike '%pontual%' or prompt ilike '%compromiss%' or prompt ilike '%aprend%' or prompt ilike '%constân%' or prompt ilike '%constan%' or prompt ilike '%regular%' or prompt ilike '%calma%'
    then 'mental'
  when prompt ilike '%veloc%' or prompt ilike '%acelera%' or prompt ilike '%força%' or prompt ilike '%forca%' or prompt ilike '%resist%' or prompt ilike '%explos%' or prompt ilike '%agilid%'
    then 'fisico'
  else 'tecnica'
end;

create index if not exists technical_monthly_questions_season_pillar_idx
on public.technical_monthly_questions (season_id, pillar);
