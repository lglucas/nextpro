...
export function BaseQuestionsList({
  questions,
  onToggleActive,
  onDelete,
}: {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
      <p className="text-sm font-semibold text-slate-900">Base (slot 1)</p>
      <div className="mt-3 space-y-2">
        {questions.length === 0 ? <p className="text-sm text-slate-500">Nenhuma pergunta base cadastrada.</p> : null}
        {questions.map((q) => (
